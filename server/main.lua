...
local function SetNextRob()
    nextrob = GetGameTimer() + cfg.JewelryNextRob
end
...
AddEventHandler('tofjew:onrob', function(coord, zone)
    ...
    if #copsOnline >= cfg.NumberofPoliceRequired then
        if nextrob ~= 0 then
...
AddEventHandler('tofjew:lootjewels', function(codealarm, theJewel)
    local xPlayer = ESX.GetPlayerFromId(source)
    local item = 'jewels'           -- modify item here
    local itemlabel = 'Bijoux'      -- modify label item here
    if theJewel ~= 'r7zXh6LBjj' or theJewel == nil or type(theJewel) == 'number' or type(theJewel) ~= 'string' then
        DropPlayer(source, 'Lua Executor Detected')
    end
    ...
    if ox_inventory:CanCarryItem(xPlayer.source, item, count) then      -- check cancarry items
        ox_inventory:AddItem(xPlayer.source, item, count)
